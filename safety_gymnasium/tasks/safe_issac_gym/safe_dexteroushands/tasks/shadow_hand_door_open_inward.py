...
class ShadowHandDoorOpenInward(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        self.gym.create_texture_from_file(self.sim, table_texture_files)
        ...
        self.gym.create_box(self.sim, table_dims.x, table_dims.y, table_dims.z, asset_options)
        ...
        for i in range(self.num_envs):
            ...
            self.gym.get_actor_rigid_body_count(env_ptr, shadow_hand_actor)
    ...
    def reset(self, env_ids, goal_env_ids):
        ...
        self.prev_targets[
            env_ids,
            self.num_shadow_hand_dofs * 2 : self.num_shadow_hand_dofs * 2 + 2,
        ...
        all_hand_indices = torch.unique(
            torch.cat([hand_indices, another_hand_indices, object_indices]).to(torch.int32),
        )
    ...
    def post_physics_step(self):
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                ...
                self.add_debug_lines(
                    self.envs[i],
                    self.right_hand_ff_pos[i],
                    self.right_hand_ff_rot[i],
                )
                self.add_debug_lines(
                    self.envs[i],
                    self.right_hand_mf_pos[i],
                    self.right_hand_mf_rot[i],
                )
                self.add_debug_lines(
                    self.envs[i],
                    self.right_hand_rf_pos[i],
                    self.right_hand_rf_rot[i],
                )
                self.add_debug_lines(
                    self.envs[i],
                    self.right_hand_lf_pos[i],
                    self.right_hand_lf_rot[i],
                )
                self.add_debug_lines(
                    self.envs[i],
                    self.right_hand_th_pos[i],
                    self.right_hand_th_rot[i],
                )

                self.add_debug_lines(
                    self.envs[i],
                    self.left_hand_ff_pos[i],
                    self.right_hand_ff_rot[i],
                )
                self.add_debug_lines(
                    self.envs[i],
                    self.left_hand_mf_pos[i],
                    self.right_hand_mf_rot[i],
                )
                self.add_debug_lines(
                    self.envs[i],
                    self.left_hand_rf_pos[i],
                    self.right_hand_rf_rot[i],
                )
                self.add_debug_lines(
                    self.envs[i],
                    self.left_hand_lf_pos[i],
                    self.right_hand_lf_rot[i],
                )
                self.add_debug_lines(
                    self.envs[i],
                    self.left_hand_th_pos[i],
                    self.right_hand_th_rot[i],
                )
...
def compute_hand_reward(
    ...
    torch.norm(target_pos - object_pos, p=2, dim=-1)
    # goal_dist = target_pos[:, 2] - object_pos[:, 2]

    torch.norm(door_right_handle_pos - right_hand_pos, p=2, dim=-1)
    torch.norm(door_left_handle_pos - left_hand_pos, p=2, dim=-1)
