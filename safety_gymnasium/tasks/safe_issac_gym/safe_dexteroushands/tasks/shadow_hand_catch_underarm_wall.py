...
class ShadowHandCatchUnderarmWall(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        self.gym.create_texture_from_file(self.sim, wall_texture_files)
    ...
    def compute_cost(self, actions):
        ...
        collision = (
            (torch.abs(self.object_pos[:, 0]) > 0.25)
            | (self.object_pos[:, 1] > 0.70)
            | (self.object_pos[:, 1] < 0.60)
        )
...
def compute_hand_reward(
    ...
    success_reward = torch.where(goal_resets == 1, reach_goal_bonus, 0.0)

    # Fall penalty: distance to the goal is larger than a threshold
    fall_reward = torch.where(object_pos[:, 2] <= 0.1, fall_penalty, 0.0)
    ...
    if max_consecutive_successes > 0:
        penalty_reward = torch.where(progress_buf >= max_episode_length, 0.5 * fall_penalty, 0.0)
        reward += penalty_reward
        print('penalty_reward', penalty_reward.mean().item())
        reward_detail = {
            'total reward': reward.mean(),
            'dist reward': dist_reward.mean(),
            'success_reward': success_reward.mean(),
            'fall_reward': fall_reward.mean(),
            'penalty_reward': penalty_reward.mean(),
        }
    else:
        reward_detail = {
            'total reward': reward.mean(),
