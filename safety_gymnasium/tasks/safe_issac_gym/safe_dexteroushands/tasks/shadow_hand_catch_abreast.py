...
def compute_hand_reward(
    ...
    right_hand_base_dist = torch.norm(
        right_hand_base_pos - torch.tensor([-0.3, -0.55, 0.5], dtype=torch.float, device='cuda:0'),
        p=2,
        dim=-1,
    )
    left_hand_base_dist = torch.norm(
        left_hand_base_pos - torch.tensor([-0.3, -1.15, 0.5], dtype=torch.float, device='cuda:0'),
        p=2,
        dim=-1,
    )
