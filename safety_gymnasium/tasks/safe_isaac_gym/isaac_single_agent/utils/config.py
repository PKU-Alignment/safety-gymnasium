...
def retrieve_cfg(args, use_rlg_config=False):
    ...
    elif args.task == 'FreightFrankaCloseDrawer':
        return (
            os.path.join(
                args.logdir,
                f'ShadowHandCatchOver2underarm_Safe_finger/{args.algo}/{args.algo}',
            ...
            f'cfg/{args.algo}/config.yaml',
            'cfg/freight_franka_close_drawer.yaml',
        )
    elif args.task == 'FreightFrankaPickAndPlace':
        return (
            os.path.join(
                args.logdir,
                f'ShadowHandCatchOver2underarm_Safe_joint/{args.algo}/{args.algo}',
            ...
            f'cfg/{args.algo}/config.yaml',
            'cfg/freight_franka_pick_and_place.yaml',
        )
    else:
        warn_task_name()
...
def get_args(benchmark=False, use_rlg_config=False):
    custom_parameters = [
        ...
        {'name': '--cost_lim', 'type': float, 'default': 25, 'help': 'cost_lim'},
    ]
    ...
    # TODO
    if type(args.rl_device) is str and args.rl_device.startswith('cuda:'):
        args.device_id = args.rl_device
    else:
