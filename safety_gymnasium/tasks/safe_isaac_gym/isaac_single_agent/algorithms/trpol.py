...
class TRPOL:
    def __init__(
        ...
        self.critic = critic_class(
            self.observation_space.shape, self.state_space.shape, model_cfg, asymmetric=asymmetric
        )
    ...
    def log(self, locs, width=80, pad=35):
        ...
        if len(locs['costbuffer']) > 0:
            ...
            self.writer.add_scalar(
                'Train/mean_cost/time', statistics.mean(locs['costbuffer']), self.tot_time
            )
            self.writer.add_scalar(
                'Train/mean_episode_length/time', statistics.mean(locs['lenbuffer']), self.tot_time
            )
    ...
    def update(self, avg_cost):
        ...
        for epoch in range(self.num_learning_epochs):
            for indices in batch:
                ...
                def Fvp(v):
                    kl = gaussian_kl(
                        mean_batch, std_batch, mean_batch.detach(), std_batch.detach()
                    ).mean()
                    self.actor.zero_grad()
                ...
                def set_and_eval(step):
                    ...
                    kl_value = gaussian_kl(
                        mean_eval, std_eval, mean_batch.detach(), std_batch.detach()
                    ).mean()
                    # print("kl", kl_value)
