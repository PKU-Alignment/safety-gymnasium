...
from safety_gymnasium.tasks.multiagent_mujoco.coupled_half_cheetah import CoupledHalfCheetahEnv
from safety_gymnasium.tasks.multiagent_mujoco.many_segment_ant import ManySegmentAntEnv
from safety_gymnasium.tasks.multiagent_mujoco.many_segment_swimmer import ManySegmentSwimmerEnv
from safety_gymnasium.tasks.multiagent_mujoco.obsk import (
    Node,
...
class MultiAgentMujocoEnv(pettingzoo.utils.env.ParallelEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if scenario in _MUJOCO_GYM_ENVIROMENTS:
            self.single_agent_env: gymnasium.envs.mujoco.mujoco_env.MujocoEnv = gymnasium.make(
                scenario, **kwargs, render_mode=render_mode
            )
        ...
        if self.agent_obsk is not None:
            ...
            self.agent_action_partitions = [
                tuple(dummy_node for i in range(self.single_agent_env.action_space.shape[0]))
            ]
            mujoco_edges = []

        # Create agent lists
        self.possible_agents = [
            'agent_' + str(agent_id) for agent_id in range(len(self.agent_action_partitions))
        ]
    ...
    def map_local_actions_to_global_action(self, actions: dict[str, np.ndarray]) -> np.ndarray:
        ...
        for agent_id, partition in enumerate(self.agent_action_partitions):
            for act_index, body_part in enumerate(partition):
                ...
                global_action[body_part.act_ids] = actions[self.possible_agents[agent_id]][
                    act_index
                ]

        assert not np.isnan(global_action).any(), 'FATAL: At least one gym_env action is undefined!'
        return global_action

    def map_global_action_to_local_actions(self, action: np.ndarray) -> dict[str, np.ndarray]:
    ...
    def map_global_state_to_local_observations(
        ...
        mujoco_data = data_struct(
            ...
            qfrc_actuator=np.array(global_state[cvel_end_index:qfrc_actuator_end_index]),
        ...
        if len(mujoco_data.cvel) != 0:
            mujoco_data.cvel = np.reshape(mujoco_data.cvel, self.single_agent_env.data.cvel.shape)
        ...
        assert len(self.single_agent_env.unwrapped.data.qpos.flat) == len(mujoco_data.qpos)
        assert len(self.single_agent_env.unwrapped.data.qvel.flat) == len(mujoco_data.qvel)
    ...
    def _generate_local_categories(self, scenario: str) -> list[list[str]]:
        ...
        categories = [
            k_split[k if k < len(k_split) else -1].split(',') for k in range(self.agent_obsk + 1)
        ]
        return categories
