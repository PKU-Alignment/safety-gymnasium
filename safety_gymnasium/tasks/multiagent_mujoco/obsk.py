    ...
    def _adjacent(lst):  # return all sets adjacent to any element in lst
        ...
        for element in lst:
            ret = ret.union(
                set(
                    itertools.chain(
                        *[e.nodes.difference({element}) for e in hyperedges if element in e]
                    )
                )
            )
        return ret
...
def get_parts_and_edges(  # noqa: C901
    ...
    if label in ['HalfCheetah-v4']:
        ...
        thigh_joint = Node(
            ...
            -3,
            0,
            extra_obs={'qvel': lambda data: np.clip(np.array([data.qvel[-3]]), -10, 10)},
        )
        leg_joint = Node(
            'leg_joint',
            -2,
        ...
        target_x = Node('target_x', -2, -2, None, extra_obs={'qvel': (lambda data: np.array([]))})
        target_y = Node('target_y', -1, -1, None, extra_obs={'qvel': (lambda data: np.array([]))})
        ...
        root_x0 = Node('root_x0', 0, 0, None, extra_obs={'qpos': lambda data: np.array([])})
        root_z0 = Node('root_z0', 1, 1, None)
        root_y0 = Node('root_y0', 2, 2, None)
        root_x1 = Node('root_x1', 9, 9, None, extra_obs={'qpos': lambda data: np.array([])})
        ...
        # define Mujoco-Graph
        joints = [Node(f'rot{i:d}', -n_segs + i, -n_segs + i, i) for i in range(0, n_segs)]
