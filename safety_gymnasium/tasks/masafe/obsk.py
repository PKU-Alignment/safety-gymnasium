...
def get_joints_at_kdist(agent_id, agent_partitions, hyperedges, k=0, kagents=False):
    ...
    for _k in range(k + 1):
        ...
        seen = seen.union(new)
        k_dict[_k] = sorted(new, key=lambda x: x.label)
    return k_dict
            ...
            for c in cats:
                if c in _t.extra_obs:
                    ...
                    if c in ['qvel', 'qpos']:  # this is a "joint position/velocity" item
                        items = getattr(env.data, c)[getattr(_t, f'{c}_ids')]
                        obs_lst.extend(items if isinstance(items, list) else [items])
    ...
    for c in global_categories:
        if c in ['qvel', 'qpos']:  # this is a "joint position" item
            for j in global_dict.get('joints', []):
                items = getattr(env.data, c)[getattr(j, f'{c}_ids')]
                obs_lst.extend(items if isinstance(items, list) else [items])
        else:
...
def get_parts_and_edges(label, partitioning):
    if label in ['half_cheetah', 'HalfCheetah-v4']:
        ...
        free_joint = Node(
            ...
            -1,
            extra_obs={
                'qpos': lambda env: env.data.qpos[:7],
        ...
        joint0 = Node(
            ...
            extra_obs={
                'qpos': (
                    lambda env: np.array([np.sin(env.data.qpos[-4]), np.cos(env.data.qpos[-4])])
                ),
            },
        )
        joint1 = Node(
            ...
            extra_obs={
                ...
                'qpos': (
                    lambda env: np.array([np.sin(env.data.qpos[-3]), np.cos(env.data.qpos[-3])])
                ),
            },
        )
        ...
        bthigh = Node(
            ...
            tendons=[tendon],
            extra_obs={
                'ten_J': lambda env: env.data.ten_J[tendon],
